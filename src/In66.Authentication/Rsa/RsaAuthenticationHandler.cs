...
{
    /// <summary>
    /// RsaAuthenticationHandler
    /// </summary>
    public class RsaAuthenticationHandler : AuthenticationHandler<RsaSchemeOptions>
    {
        private AbstractAuthenticationProcessor _authenticationProcessor;
        public RsaAuthenticationHandler(
            IOptionsMonitor<RsaSchemeOptions> options,
        ...
        #region Private
        private  Lazy<ICacheProvider> _cacheProvider { get; set; }
        ...
        public static long CurrentTimeMillis() => (DateTime.UtcNow.Ticks - UtcStartTicks) / 10000L;
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            try
            {
                ...
                //请求时间大于5分钟的就抛弃
                if (Math.Abs(CurrentTimeMillis() - timestamp) > 5 * 60 * 1000)
                ...
                if (!string.IsNullOrEmpty(_cacheProvider.Value.Get<string>(redisKey).Value))
                {
                    return await AuthenticateResultFailAsync("请勿重复提交");
                }
                else
                {
                    _cacheProvider.Value.Set(redisKey, nonce, TimeSpan.FromMinutes(5));
                }
                ...
                //数据库获取
                var app = await _authenticationProcessor.GetAppInfoAsync(appid);

                if (app == null)
                    return await AuthenticateResultFailAsync("未找到对应的应用信息");
                ...
                var PublickKey = RsaKeyConvert.RSAPublicKeyJava2DotNet(app.PublicKey);
                var signResult = SecurityExtentions.VerifyCSharp($"{signBodyString}", PublickKey, signature);
                ...
                var identity = new ClaimsIdentity(RsaDefaults.AuthenticationScheme);
                identity.AddClaim(new Claim("appid", app.AppId));
                identity.AddClaim(new Claim("appname", app.AppName));
                identity.AddClaim(new Claim("role", "app"));
                var principal = new ClaimsPrincipal(identity);
                authResult=AuthenticateResult.Success(new AuthenticationTicket(principal, new AuthenticationProperties(), Scheme.Name));

                var validatedContext = new RsaValidatedContext(Context, Scheme, Options)
                {
                };
