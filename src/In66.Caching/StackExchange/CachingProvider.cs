namespace In66.Caching.StackExchange
{
    ...
    public partial class CachingProvider : AbstracCacheProvider, ICacheProvider
    {
        ...
        public override IOptions<RedisOptions> RedisOptions => _RedisOptions;
        ...
        public override string Name => RedisConstValue.Provider.StackExchange;
        ...
        /// The serializer.
        /// </summary>
        public override IRedisSerializer Serializer => _serializer;
        ...
        /// <param name="RedisOptions">RedisOptions.</param>
            ...
            ILoggerFactory loggerFactory = null)
        {
            ...
            this._serializer = !string.IsNullOrWhiteSpace(RedisOptions.Value.SerializerName)
                                       ? serializers.Single(x => x.Name.Equals(RedisOptions.Value.SerializerName))
        ...
        protected override ReValue<T> BaseGet<T>(string cacheKey, Func<T> dataRetriever, TimeSpan expiration)
        {
            ...
            {
                var exists = _redisDb.BfExistsAsync(_RedisOptions.Value.PenetrationSetting.BloomFilterSetting.Name, cacheKey).GetAwaiter().GetResult();
                if (!exists)
                {
            ...
            var flag = _redisDb.Lock(cacheKey, _RedisOptions.Value.LockMs / 1000);
            if (!flag.Success)
            {
                Thread.Sleep(_RedisOptions.Value.SleepMs);
        ...
        /// <param name="cacheKey">Cache key.</param>
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected override ReValue<T> BaseGet<T>(string cacheKey)
        {
            ArgumentCheck.NotNullOrWhiteSpace(cacheKey, nameof(cacheKey));
        ...
        /// <param name="expiration">Expiration.</param>
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected override void BaseSet<T>(string cacheKey, T ReValue, TimeSpan expiration)
        {
            ArgumentCheck.NotNullOrWhiteSpace(cacheKey, nameof(cacheKey));
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected override IDictionary<string, ReValue<T>> BaseGetAll<T>(IEnumerable<string> cacheKeys)
        {
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected override IDictionary<string, ReValue<T>> BaseGetByPrefix<T>(string prefix)
        {
            ArgumentCheck.NotNullOrWhiteSpace(prefix, nameof(prefix));
        ...
        /// <param name="expiration">Expiration.</param>
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected override bool BaseTrySet<T>(string cacheKey, T ReValue, TimeSpan expiration)
        {
            ArgumentCheck.NotNullOrWhiteSpace(cacheKey, nameof(cacheKey));
            ArgumentCheck.NotNull(ReValue, nameof(ReValue));
            ArgumentCheck.NotNegativeOrZero(expiration, nameof(expiration));
