    ...
    public class AuthSecurityRsaAuthenticationHandler : AuthenticationHandler<AuthSecurityRsaOptions>
    {
        ...
        protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            try
            {
                ...
                //这里我们抛开小明等人谈论nonce和timestamp，而以熟知的web应用来举例。首先，客户端请求前生成一个随机数nonce，作为该请求的唯一标识，签名时加入该nonce以保证nonce不被篡改，同时由于签名中加入了随机数nonce，因此相同的请求参数，每次签名得出的结果都不同，保证了签名不可预测，nonce放在header中一起发送给服务端，以便服务端使用该nonce进行验签，并记录起来。
