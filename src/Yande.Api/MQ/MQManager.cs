...
namespace OfficeToPdf
{
    public class MQManager
    {
        ...
        private static void Open(MQConfig config)
        {
            try
            {
                ...
                lock (LockObj)
                {
                    ...
                    factory.NetworkRecoveryInterval = config.NetworkRecoveryInterval;

                    List<AmqpTcpEndpoint> list = new List<AmqpTcpEndpoint>();
                    //多host用,分隔
                    for (int i = 0; i < config.Host.Split(',').Length; i++)
                    {
                        string host = config.Host.Split(',')[i];
                        string port = config.Port.Split(',')[i];
                    ...
                    _connection = _connection ?? factory.CreateConnection(list);
                }
            }
            catch (Exception ex)
            {
        ...
        public void Publish(Messages message)
        {
            ...
            //byte[] messByte = JsonSerializer.SerializeToUtf8Bytes<Messages>(message);

            //byte[] messByte = MessagePackSerializer.Serialize(message);

            var messByte = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(message));
